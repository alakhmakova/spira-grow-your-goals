  ...
  <AlertDialogPortal>
    ...
    <AlertDialogPrimitive.Content
      ...
      className={cn(
        ...
        "inset-x-0 bottom-0 p-4 rounded-t-lg max-h-[85vh] overflow-y-auto",
        // Desktop: centered modal
        "sm:inset-auto sm:left-[50%] sm:top-[50%] sm:translate-x-[-50%] sm:translate-y-[-50%] sm:max-w-lg sm:p-6 sm:rounded-lg",
...
const AlertDialogFooter = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div className={cn("flex flex-row justify-between items-center gap-2", className)} {...props} />
);
...
>(({ className, ...props }, ref) => (
  <AlertDialogPrimitive.Description ref={ref} className={cn("text-sm proton-paragraph", className)} {...props} />
));
...
>(({ className, variant, size, ...props }, ref) => (
  <AlertDialogPrimitive.Action
    ref={ref}
    className={cn(buttonVariants({ variant, size: size ?? "sm", className }))}
    {...props}
  />
));
...
>(({ className, ...props }, ref) => (
  <AlertDialogPrimitive.Cancel
    ref={ref}
    className={cn(buttonVariants({ variant: "cancel", size: "sm" }), className)}
    {...props}
  />
));
